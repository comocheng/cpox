...
def import_data(ratio, file_location=False):
    ...
    if file_location is False:
        ...
        data = pd.read_csv('./large-grid/' + file_location + '/data.csv')
...
array = os.listdir('./large-grid/')  # find the LSR folders
array = sorted(array)
...
for z in range(len(all_data)):
    ...
    for s in range(len(ans[0])):
        ...
        lavaPlot(data_to_plot, title, axis=spans[s], folder='base-large-grid', interpolation=False)  # making plots
...
def import_sensitivities(ratio, file_location=False, thermo=False):
    ...
    try:
        ...
        if file_location is False:
            ...
            if thermo is False:
                data = pd.read_csv('./large-grid/' + file_location + '/sensitivities/' + str(ratio) + 'RxnSensitivity.csv')
            else:
                data = pd.read_csv('./large-grid/' + file_location + '/sensitivities/' + str(ratio) + 'ThermoSensitivity.csv')
        data = data.get_values()
        data = data.tolist()
        return data
    except:
        ...
        for r in try_ratios:
            try:
                if file_location is False:
                    ...
                    if thermo is False:
                        data = pd.read_csv('./large-grid/' + file_location + '/sensitivities/' + str(r) + 'RxnSensitivity.csv')
                    else:
                        data = pd.read_csv('./large-grid/' + file_location + '/sensitivities/' + str(r) + 'ThermoSensitivity.csv')
                data = data.get_values()
                fakedata = data
...
#             all_sens = []
            ...
            for r in range(len(all_data[0])):  # for a single ratio
                ...
                if skip is True:
                    ...
                    sensPlot(sensitivities, title, folder='sensitivities-large-grid', axis=[-1*MAX, MAX])
    return most_sens
